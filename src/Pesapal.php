    ...
    public static function createToken(): ?PesapalToken
    {
        ...
        if ($response->ok()) {
            return PesapalToken::create([
                ...
                'expires_at' => Carbon::parse(time: $response->json(key: 'expiryDate'), timezone:  'UTC')->tz(config(key: 'app.timezone'))
            ]);
        }
        ...
        if ($response->ok()) {
            return PesapalIpn::updateOrCreate(
                ...
                values: [
                    ...
                    // Then, the timezone is converted to the application's timezone using the 'tz' method.
                    'created_at' => Carbon::parse(time: $response->json(key: 'created_date'), timezone:  'UTC')->tz(config(key: 'app.timezone'))
                ]
            );
        }
